	...
	if edge_path_index:
		...
		if len(no_access.index) > 0:
			for iter_,value in no_access.iterrows():
				fail_dict = {'edge_id':str(edge_failure_set),'econ_value':value[rev_criteria],'tons':value[tons_criteria],
		...
		if len(po_access.index) > 0:
			...
			for origin in origins:
				...
				if len(po_access.loc[origin].shape) == 1:
					...
					# print (destinations,croptons,rev,veh_nums,dist,time,gcost)
					...
					# print (po_access.loc[origin])
					# print (len(po_access.loc[origin].index))
					...
					for p in range(len(paths)):
						if len(paths[p]) > 0:
							...
							fail_dict = {'edge_id':str(edge_failure_set),'econ_value':rev[p],'tons':croptons[p],'vehicle_nums':veh_nums[p],
			...
			# for t in range(1,2):
				...
				for edge_fail in multiple_ef_list:
					if edge_fail[0] not in e_list:
						ef_dict = igraph_scenario_edge_failures(G_df,edge_fail[0],flow_df,tr_wt,path_types[t],rev_types[t],tons_types[t],cost_types[t],dist_types[t],time_types[t],vechicle_types[t])
						if ef_dict:
							ef_list += ef_dict
							e_list.append(edge_fail[0])
							ef_sc_list += [{**ef,**edge_fail[1]} for ef in ef_dict]
					elif ef_list:
				...
				egde_impact = df[filter_cols + ['econ_value','tons','econ_loss']]
				egde_impact = egde_impact.groupby(filter_cols)['econ_value','tons','econ_loss'].sum().reset_index()
